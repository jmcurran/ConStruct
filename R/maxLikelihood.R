...
#' maxLikelihood(data.f, resolution = 100)
maxLikelihood = function(dataf, max.alleles, resolution) {
  ...
  f = seq(0, fst.max, length = resolution)
  ...
  G = exp(log(max(probability)) - 2)

  cat(paste0("G = ", signif(G, 6), "\n"))


  invisible(list(fst = ML, fst.max = fst.max, G = 0))
}
# end of function
